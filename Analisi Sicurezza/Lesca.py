...
def initialization(secret, key):
    ...
    #Divisione dell'output della hash function in 4 parti da 96 bit ed in 1 da 128 bit
    #per la seguente generazione delle tabelle di permutazione e del vettore Key Stream
    k1= d[0:96]
    k2= d[96:192]
    k3= d[192:288]
    k4= d[288:384]
...
def R(V, Value1, Value2):
    """Funzione di update del vettore di key stream, definita in Speck cipher, per cui
    perturbiamo il valore di V effettuando operazioni semplici usando V stesso, Value1 e Value2."""
    V = np.uint64(ROR(V, 8))
...
def permutation(perm1, perm2):
    ...
    In base al valore di perm2 in posizione i, sposto il valore di perm1 a destra con x=perm2[i], creando
    così una permutazione. Se ho già scritto un valore nella cella, scorro la lista a destra fino a quando
    non trovo una cella vuota e metto lì il valore."""
    res = [-1]*len(perm1)
...
def update_procedure(V, perm1, perm2, perm3, perm4):
    """Funzione che effettua l'update del vettore di key stream e
    delle varie tabelle di permutazione grazie alla funzione R ed alla funzione permutation.
    Chiamata da round_func quando j=DELTA."""
    #aggiorniamo V tramite la funzione R (proveniente da Simon & Speck cipher)
    for i in range(0, NUMBER_OF_WORDS):
...
def round_func (plain, V, perm1, perm2, perm3, perm4, nb):
    ...
    for j in range(1,nb+1):
        ...
        # 2. Usiamo il valore calcolato precedentemente come seed per un PRNG, che ci dà un valore intermedio per il key stream in posizione J. 
        # Applichiamo questo processo a tutti i membri di X, ovvero g=dim(X)=dim(V)
        ...
        resulting_text = []
        #applica la XOR tra il valore contenuto in V in posizione j ed il plaintext j
        # len(intermediate)/64 gestisce il caso in cui nell'ultimo blocco non ci siano esattamente h blocchi, ma
        # di meno, il casting ad int è necessario per far funzionare il tutto, altrimenti, se il valore è float, il codice non funziona
...
def lesca_decrypt(msg, V, perm1, perm2, perm3, perm4):
    ...
    res = ''.join(res)
    #trasformo la stringa di bit in una stringa di caratteri
    final_string = []
    for i in range (0,int(len(res)/WORD_PRECISION)):
        final_string.append(chr(int(res[i*WORD_PRECISION:(i+1)*WORD_PRECISION],2)))
...
#Creazione Sessione Key a 512 bits tramite Elliptic Curve Diffie Hellman Ephemeral (NB: nel codice in shared_session_key.py manca l'implementazione della comunicazione tra i due dispositivi, ma la simula)
